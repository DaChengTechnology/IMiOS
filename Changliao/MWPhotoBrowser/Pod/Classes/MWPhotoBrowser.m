...
- (void)viewDidLoad {
    ...
    if (_enableSwipeToDismiss) {
        UISwipeGestureRecognizer *swipeGesture = [[UISwipeGestureRecognizer alloc] initWithTarget:self action:@selector(DownButtonPressed:)];
...
- (void)performLayout {
    ...
    if ([self.navigationController.viewControllers objectAtIndex:0] == self) {
        ...
        _doneButton = [[UIBarButtonItem alloc] initWithTitle:@"保存" style:UIBarButtonItemStylePlain target:self action:@selector(doneButtonPressed:)];
        // Set appearance
        ...
        _downButton = [[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"Done",@"Done") style:UIBarButtonItemStylePlain target:self action:@selector(DownButtonPressed:)];
        // Set appearance
        [_downButton setBackgroundImage:nil forState:UIControlStateNormal barMetrics:UIBarMetricsDefault];
        [_downButton setBackgroundImage:nil forState:UIControlStateNormal barMetrics:UIBarMetricsLandscapePhone];
        [_downButton setBackgroundImage:nil forState:UIControlStateHighlighted barMetrics:UIBarMetricsDefault];
        [_downButton setBackgroundImage:nil forState:UIControlStateHighlighted barMetrics:UIBarMetricsLandscapePhone];
        [_downButton setTitleTextAttributes:[NSDictionary dictionary] forState:UIControlStateNormal];
        [_downButton setTitleTextAttributes:[NSDictionary dictionary] forState:UIControlStateHighlighted];
        
        self.navigationItem.leftBarButtonItem = _downButton;
...
- (void)doneButtonPressed:(id)sender {
    // Only if we're modal and there's a done button
        if([_savedelegate respondsToSelector:@selector(SaveButtonWithIndex:)]){
            [_savedelegate SaveButtonWithIndex:_currentPageIndex];
        }
}

#pragma mark - DownAction
...
- (void)actionButtonPressed:(id)sender {
    ...
    if ([self numberOfPhotos] > 0 && [photo underlyingImage]) {
        ...
        if ([self.delegate respondsToSelector:@selector(photoBrowser:actionButtonPressedForPhotoAtIndex:)]) {
            ...
            self.activityViewController.modalPresentationStyle = UIModalPresentationOverFullScreen;
