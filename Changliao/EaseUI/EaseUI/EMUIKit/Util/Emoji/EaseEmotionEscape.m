...
{
    return [[EaseEmotionEscape sharedInstance] attributtedStringFromText:aInputText];
}
...
+(NSString*) stringForInputView:(NSString*)aInputText {
    return [[EaseEmotionEscape sharedInstance] strinFromInputView:aInputText];
}

-(NSString*) strinFromInputView:(NSString*)aInputText {
    if (_urlPattern == nil || _urlPattern.length == 0) {
        return aInputText;
    }
    NSString *urlPattern = _urlPattern;
    ...
    NSArray* matches = [regex matchesInString:aInputText options:NSMatchingReportCompletion range:NSMakeRange(0, [aInputText length])];
    NSMutableString * string = [[NSMutableString alloc] initWithString:aInputText];
    ...
    for (NSTextCheckingResult *match in [matches reverseObjectEnumerator]) {
        ...
        if (emojiName != nil || emojiName.length > 0) {
            [string deleteCharactersInRange:matchRange];
            [string insertString:@"[表情]" atIndex:matchRange.location];
        }
    }
