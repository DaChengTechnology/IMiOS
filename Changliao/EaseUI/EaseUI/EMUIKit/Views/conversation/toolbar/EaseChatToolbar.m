...
@property (nonatomic) BOOL isOnVoice;
@property (nonatomic) BOOL isNeedHideVoice;
...
//    [self addSubview:_backgroundImageView];
    self.backgroundColor = kColorFromRGBHex(0xf6f6f6);
    //toolbar
    ...
    [styleChangeButton setImage:[UIImage imageNamed:@"voiceBtn"] forState:UIControlStateNormal];
    ...
    [self.faceButton setImage:[UIImage imageNamed:@"笑脸"] forState:UIControlStateNormal];
    [self.faceButton setImage:[UIImage imageNamed:@"笑脸"] forState:UIControlStateHighlighted];
    [self.faceButton setImage:[UIImage imageNamed:@"键盘"] forState:UIControlStateSelected];
    ...
    [self.moreButton setImage:[UIImage imageNamed:@"聊天框加号"] forState:UIControlStateNormal];
    [self.moreButton setImage:[UIImage imageNamed:@"聊天框加号"] forState:UIControlStateHighlighted];
...
- (void)setInputViewLeftItems:(NSArray *)inputViewLeftItems
{
    ...
    CGFloat itemHeight = [EaseChatToolbar defaultHeight] - self.verticalPadding * 2;
    for (id item in inputViewLeftItems) {
        if ([item isKindOfClass:[EaseChatToolbarItem class]]) {
            ...
            if (chatItem.button) {
                ...
                itemFrame.origin.y = (self.toolbarView.frame.size.height - itemFrame.size.height - self.verticalPadding);
...
- (void)_willShowInputTextViewToHeight:(CGFloat)toHeight
{
    ...
    if (toHeight == _previousTextViewContentHeight)
    {
        ...
        [self.inputTextView setTextAlignment:NSTextAlignmentLeft];
...
//        return NO;
//    }
//    else if ([self.delegate respondsToSelector:@selector(textView:shouldChangeTextInRange:replacementText:)]) {
//        //
//        return [self.delegate textView:self.inputTextView shouldChangeTextInRange:range replacementText:text];
//    }
...
//    return YES;
    ...
    if ([text isEqualToString:@"\n"]) {
        if (_inputTextView.attributedText.length < 1) {
            return NO;
        }
        if ([self.delegate respondsToSelector:@selector(didSendText:)]) {
            //转义回来，否则回车发送回显示空
...
- (void)showVoiceStyleButton:(BOOL)isManager {
    if (isManager) {
        if ([self.leftItems count] == 0 || [self.leftItems count] == nil) {
            UIButton *styleChangeButton = [[UIButton alloc] init];
            styleChangeButton.accessibilityIdentifier = @"style";
            styleChangeButton.autoresizingMask = UIViewAutoresizingFlexibleTopMargin;
            ...
            EaseChatToolbarItem *styleItem = [[EaseChatToolbarItem alloc] initWithButton:styleChangeButton withView:nil];
            [self _willShowBottomHeight:[EaseChatToolbar defaultHeight] - self.frame.size.height];
            [self setInputViewLeftItems:@[styleItem]];
        ...
        if ([self.leftItems count] > 0) {
            [self setInputViewLeftItems:[NSArray array]];
            [self _willShowBottomHeight:[EaseChatToolbar defaultHeight] - self.frame.size.height];
            if (!self.isOnVoice) {
                self.recordButton.hidden = true;
                self.inputTextView.hidden = false;
            }else{
                self.isNeedHideVoice = true;
            }
        }
    }
}
    ...
    if (!isDelete && str.length > 0) {
        ...
        if (self.version >= 7.0) {
            if (chatText.length > 0) {
                NSInteger length = 1;
                BOOL isEmoji = NO;
                if ([EaseEmoji stringContainsEmoji:[chatText substringFromIndex:chatText.length - 1]]) {
                    isEmoji = YES;
                }
                if (chatText.length >= 2 && !isEmoji) {
