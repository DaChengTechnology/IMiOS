    ...
    if ([self respondsToSelector:@selector(isCustomBubbleView:)] && [self isCustomBubbleView:model]) {
        ...
        switch (messageType) {
            case EMMessageBodyTypeText:
            {
                if (bm) {
                    if (bm.isIDCard) {
                        [_bubbleView setupIDCardBubbleView];
                        if (bm.isSender) {
                            [_bubbleView setIDCardBubbleView];
                        }else{
                            [_bubbleView setIDCardReciveBubbleView];
                        }
                    }else{
            ...
            {
                _bubbleMaxWidth = kEMMessageImageSizeWidth;
                _bubbleMaxWidth += 16;
...
{
    //bubble view
    [self addConstraint:[NSLayoutConstraint constraintWithItem:self.bubbleView attribute:NSLayoutAttributeBottom relatedBy:NSLayoutRelationLessThanOrEqual toItem:self.contentView attribute:NSLayoutAttributeBottom multiplier:1.0 constant:kSCRATIO(-6)]];
    ...
    if ([self respondsToSelector:@selector(isCustomBubbleView:)] && [self isCustomBubbleView:model]) {
        ...
        switch (model.bodyType) {
            case EMMessageBodyTypeText:
            {
                ...
                if (bm) {
                    if (bm.isIDCard) {
                        if (bm.personalHeadURL) {
                            [_bubbleView.userIMg sd_setImageWithURL:[NSURL URLWithString:bm.personalHeadURL] placeholderImage:[UIImage imageNamed:@"moren"]];
                        }else{
                            _bubbleView.userIMg.image = [UIImage imageNamed:@"moren"];
                        }
                        _bubbleView.userNameLab.text=bm.personalName;
                        _bubbleView.userIDCardLab.text=[NSString stringWithFormat:@"%@:%@",NSLocalizedString(@"ChattingID", nil),bm.personalIDCard];
                    }else{
                        _bubbleView.textLabel.attributedText = [[EaseEmotionEscape sharedInstance] attStringFromTextForChatting:bm.text textFont:self.messageTextFont];
                    }
                }else{
            ...
            case EMMessageBodyTypeImage:
            {
                ...
                if (!image) {
                    image = _model.image;
                    if (!image) {
                        [_bubbleView.imageView sd_setImageWithURL:[NSURL URLWithString:_model.thumbnailFileURLPath] placeholderImage:[UIImage imageNamed:_model.failImageName] completed:nil];
                    } else {
            ...
            case EMMessageBodyTypeVoice:
            {
                if (_bubbleView.voiceImageView) {
                    if ([self.sendMessageVoiceAnimationImages count] > 0 && [self.recvMessageVoiceAnimationImages count] > 0) {
                        ...
                        self.bubbleView.voiceImageView.image = self.model.isSender ?[UIImage imageNamed:@"chat_sender_audio_playing_full"]: [UIImage imageNamed:@"chat_reciver_audio_playing_full"];
                    }
                }
            ...
            case EMMessageBodyTypeVideo:
            {
                ...
                [_bubbleView.videoImageView.layer setMasksToBounds:YES];
            }
                break;
            case EMMessageBodyTypeFile:
            {
        ...
        if (_bubbleView) {
            switch (_messageType) {
                case EMMessageBodyTypeText:
                {
                    ...
                    if (bm.isIDCard) {
                        [_bubbleView updateIDCardMargin:_bubbleMargin];
                    }else{
...
+ (NSString *)cellIdentifierWithModel:(id<IMessageModel>)model
{
    ...
    if (model.isSender) {
        switch (model.bodyType) {
            case EMMessageBodyTypeText:{
                NSString* p = [model.message.ext objectForKey:@"type"];
                if (p) {
                    if ([p isEqualToString:@"person"]) {
                        cellIdentifier = @"idCCardSender";
                    }else{
...
//    }
    ...
    switch (model.bodyType) {
        case EMMessageBodyTypeText:
        {
            BoxinMessageModel* bm = (BoxinMessageModel*)model;
            if (!bm) {
                NSAttributedString *text = [[EaseEmotionEscape sharedInstance] attStringFromTextForChatting:bm.text textFont:cell.messageTextFont];
        ...
        case EMMessageBodyTypeVideo:
        {
            CGFloat w = model.thumbnailImageSize.width/model.thumbnailImageSize.height*kEMMessageImageSizeHeight;
            if (w > kEMMessageImageSizeWidth) {
                height += model.thumbnailImageSize.height/model.thumbnailImageSize.width*kEMMessageImageSizeWidth;
            }else{
                height += kEMMessageImageSizeHeight;
            }
        }
            break;
        case EMMessageBodyTypeLocation:
