...
class LoginPhoneViewController: UIViewController,UITextFieldDelegate,XWCountryCodeControllerDelegate {
    ...
    func setupUI() {
        ...
        loginLabel.text = NSLocalizedString("FastLogin", comment: "Fast Login")
        ...
        let gotoPasswoedButton = UIButton(type: .custom)
        gotoPasswoedButton.setTitle(NSLocalizedString("LoginWithPassword", comment: "Login with password"), for: .normal)
        gotoPasswoedButton.addTarget(self, action: #selector(goToPasswordLogin), for: .touchUpInside)
        self.view.addSubview(gotoPasswoedButton)
        gotoPasswoedButton.mas_makeConstraints { (make) in
            make?.left.equalTo()(codeLineView)
            make?.top.equalTo()(codeLineView.mas_bottom)
            make?.width.mas_greaterThanOrEqualTo()(130.5)
    ...
    @objc func getVerifationCode() {
        self.view.endEditing(true);
            if phoneTextFeild.text == nil {
                view.makeToast(NSLocalizedString("plsInputPhoneNum", comment: "Please input your telephone number"))
                return
            }
    ...
    func login() {
        ...
        model.mobile=phoneTextFeild.text
        model.password=checkCodeTextFeild.text
        DispatchQueue.global().async {
            BoXinProvider.request(.LoginWithCheckCode(model: model), callbackQueue:  DispatchQueue.main) { (result) in
                switch result {
                case .success(let res):
                    if res.statusCode == 200 {
                        do{
