...
-(instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        _colorAngle = kColorFromRGBHex(0x8E57FF);
    }
    return self;
}
...
//
//    UIButton * btn=[UIButton CreatButtontext:@"" image:[UIImage imageNamed:@"SaobackAction"] Font:nil Textcolor:nil];
//    [self addSubview:btn];
//    [btn addTarget:self action:@selector(backClick) forControlEvents:UIControlEventTouchUpInside];
//    [btn mas_makeConstraints:^(MASConstraintMaker *make) {
//        make.left.mas_offset(kSCRATIO(10));
//        make.top.mas_offset(kSCRATIO(27-20)+kStatusBarHeight);
//        make.width.height.mas_offset(kSCRATIO(31));
//    }];
//    UILabel * titlab=[UILabel CreatLabeltext:@"扫一扫" Font:[UIFont fontWithName:@"PingFangSC-Semibold" size: kSCRATIO(17)] Textcolor:ColorWhite textAlignment:NSTextAlignmentCenter];
//    [self addSubview:titlab];
//    [titlab mas_makeConstraints:^(MASConstraintMaker *make) {
//        make.centerY.mas_equalTo(btn.mas_centerY);
//        make.left.right.mas_offset(0);
//    }];
//    UIButton * rightBtn=[UIButton CreatButtontext:@"相册" image:nil Font:[UIFont fontWithName:@"PingFangSC-Regular" size: kSCRATIO(15)] Textcolor:ColorWhite];
...
//    UILabel * tishi=[UILabel CreatLabeltext:@"将二维码放入框内，即可自动扫描可以加好友，加群，加圈子" Font:[UIFont fontWithName:@"PingFangSC-Regular" size: kSCRATIO(13)] Textcolor:kColorFromARGBHex(0xFFFFFF, 0.8) textAlignment:NSTextAlignmentCenter];
//    tishi.numberOfLines=0;
//    UIView * whitView=[[UIView alloc]init];
//    whitView.backgroundColor=ColorWhite;
//    UILabel *wode=[UILabel CreatLabeltext:@"我的二维码" Font:[UIFont fontWithName:@"PingFangSC-Regular" size: kSCRATIO(13)] Textcolor:kColorFromARGBHex(0xFFFFFF, 0.8) textAlignment:NSTextAlignmentCenter];
...
//        make.bottom.mas_equalTo(wode.mas_top).offset(kSCRATIO(-15));
//        make.left.mas_offset(kSCRATIO(128));
//        make.right.mas_offset(kSCRATIO(-128));
//        make.height.mas_offset(kSCRATIO(120));
//    }];
//    [self addSubview:tishi];
//    [tishi mas_makeConstraints:^(MASConstraintMaker *make) {
//        make.bottom.mas_equalTo(whitView.mas_top).offset(kSCRATIO(-57));
//        make.left.mas_offset(kSCRATIO(90));
//        make.right.mas_offset(kSCRATIO(-90));
//    }];
...
- (void)startScan{
    ...
    [UIView animateWithDuration:1.5 animations:^{
        weakSelf.scanLine.frame = CGRectMake(weakSelf.scanRetangleRect.origin.x, weakSelf.scanRetangleRect.origin.y + weakSelf.scanRetangleRect.size.height - kSCRATIO(176), weakSelf.scanRetangleRect.size.width, kSCRATIO(176));
    } completion:^(BOOL finished){
        if (finished == YES) {
            weakSelf.scanLine.frame = CGRectMake(weakSelf.scanRetangleRect.origin.x, kSCRATIO(128), weakSelf.scanRetangleRect.size.width, 1);
...
- (UIImageView *)scanLine{
    if (_scanLine == nil) {
        _scanLine = [[UIImageView alloc] initWithFrame:CGRectMake(self.scanRetangleRect.origin.x,kSCRATIO(128), self.scanRetangleRect.size.width,1)];
