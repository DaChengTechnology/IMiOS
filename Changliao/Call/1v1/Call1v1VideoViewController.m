...
- (void)_setupSubviews
{
    ...
    [self.switchCameraButton setImage:[UIImage imageNamed:@"switchCamera_white"] forState:UIControlStateSelected];
...
//    }];
    ...
    self.minVideoView = [[UIView alloc] initWithFrame:CGRectMake(WIDTH-kSCRATIO(110), kSCRATIO(40-20)+kStatusBarHeight, kSCRATIO(100), HEIGHT/WIDTH * kSCRATIO(80))];
    ...
    UIPanGestureRecognizer * switchTap=[[UIPanGestureRecognizer alloc]initWithTarget:self action:@selector(untip:)];
    [self.minVideoView addGestureRecognizer:switchTap];
    self.callSession.localVideoView =
    
    [[EMCallLocalView alloc] init];
...
- (void)minimizeAction
{
    ...
    UIPanGestureRecognizer * othertap=[[UIPanGestureRecognizer alloc]initWithTarget:self action:@selector(longtap:)];
    [self.floatingView addGestureRecognizer:othertap];
...
-(void)longtap:(UIPanGestureRecognizer *)tap{
    CGPoint translation = [tap translationInView:self.floatingView];
    tap.view.center = CGPointMake(tap.view.center.x + translation.x,
                                  tap.view.center.y + translation.y);
    [tap setTranslation:CGPointZero inView:self.floatingView];
}
#ifdef DEBUG
